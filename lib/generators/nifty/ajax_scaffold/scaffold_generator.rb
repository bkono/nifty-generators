...
module Nifty
  module Generators
    class ScaffoldGenerator < Base
      ...
      def create_model
        unless @skip_model
          template 'model.rb', "app/models/#{model_path}.rb" if orm == :active_record
					template 'model_mongoid.rb', "app/models/#{model_path}.rb" if orm == :mongoid
      ...
      def create_migration
        unless @skip_model || options.skip_migration? || orm_ignores_migrations?
        ...
        unless options.skip_controller?
          ...
          controller_actions.each do |action|
            if %w[index show new edit].include?(action) # Actions with templates
              ...
              template "views/#{view_language}/#{action}.js.#{view_language}", "app/views/#{plural_name}/#{action}.js.#{view_language}"
            end
						# add create.js when new is included
              template "views/#{view_language}/create.js.#{view_language}", "app/views/#{plural_name}/create.js.#{view_language}" if action == 'new'
						# add destroy.js when destroy is included
              template "views/#{view_language}/destroy.js.#{view_language}", "app/views/#{plural_name}/destroy.js.#{view_language}" if action == 'destroy'
          end
          ...
          if form_partial?
            template "views/#{view_language}/_form.html.#{view_language}", "app/views/#{plural_name}/_form.html.#{view_language}"
					...
					# include the row-item template
					template "views/#{view_language}/_model.html.#{view_language}", "app/views/#{plural_name}/_#{singular_name}.html.#{view_language}"
      ...
      def default_test_framework
        File.exist?(destination_path("spec")) ? :rspec : :testunit
			...
			def orm_ignores_migrations?
				# TODO update to include more ORMs
				@orm == :mongoid
			end
